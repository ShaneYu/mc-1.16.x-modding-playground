package com.github.shaneyu.playground.lib.client.input;
...
public abstract class KeyHandler {
     ...
     */
    private final BitSet keyRepeatings;
    ...
    public KeyHandler(Builder bindings) {
        ...
        keyRepeatings = bindings.getRepeatFlags();
        keyDown = new BitSet();
    }
    ...
    public void keyTick() {
        for (int i = 0; i < keyBindings.length; i++) {
            ...
            if (state != lastState || (state && keyRepeatings.get(i))) {
                if (state) {
